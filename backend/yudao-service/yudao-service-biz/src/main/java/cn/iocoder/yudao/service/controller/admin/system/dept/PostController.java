...
public class PostController {
    ...
    public CommonResult<Long> createPost(@Valid @RequestBody PostCreateReq reqVO) {
    ...
    public CommonResult<Boolean> updatePost(@Valid @RequestBody PostUpdateReq reqVO) {
    ...
    public CommonResult<PostResp> getPost(@RequestParam("id") Long id) {
    ...
    public CommonResult<List<PostSimpleRespVO>> getSimplePostList() {
        // 获得岗位列表，只要开启状态的
        List<SystemPost> list = postService.getPostList(null, Collections.singleton(CommonStatusEnum.ENABLE.getStatus()));
        // 排序后，返回给前端
    ...
    @PreAuthorize("@ss.hasPermission('system:post:query')")
    public CommonResult<PageResult<PostResp>> getPostPage(@Validated PostPageReqVO reqVO) {
    ...
    public void export(HttpServletResponse response, @Validated PostExportReqVO reqVO) throws IOException {
        List<SystemPost> posts = postService.getPostList(reqVO);
