...
public class RoleController {
    ...
    public CommonResult<RoleRespVO> getRole(@RequestParam("id") Long id) {
        SystemRole role = roleService.getRole(id);
    ...
    @PreAuthorize("@ss.hasPermission('system:role:query')")
    public CommonResult<PageResult<SystemRole>> getRolePage(RolePageReqVO reqVO) {
    ...
    public CommonResult<List<RoleSimpleRespVO>> getSimpleRoleList() {
        // 获得角色列表，只要开启状态的
        List<SystemRole> list = roleService.getRoleListByStatus(singleton(CommonStatusEnum.ENABLE.getStatus()));
        // 排序后，返回给前端
        list.sort(Comparator.comparing(SystemRole::sort));
    ...
    public void export(HttpServletResponse response, @Validated RoleExportReqVO reqVO) throws IOException {
        List<SystemRole> list = roleService.getRoleList(reqVO);
