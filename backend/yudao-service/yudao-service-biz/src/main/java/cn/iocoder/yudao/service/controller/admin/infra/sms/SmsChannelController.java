...
import cn.iocoder.yudao.service.vo.infra.sms.channel.*;
...
import cn.iocoder.yudao.service.service.infra.sms.SmsChannelService;
...
@Tag(name = "管理后台 - 短信渠道")
@RestController
@RequestMapping("/infra/sms-channel")
public class SmsChannelController {
    ...
    public CommonResult<SmsChannelRespVO> getSmsChannel(@RequestParam("id") Long id) {
        SystemSmsChannel smsChannel = smsChannelService.getSmsChannel(id);
    ...
    public CommonResult<PageResult<SmsChannelRespVO>> getSmsChannelPage(@Valid SmsChannelPageReqVO pageVO) {
        return success(smsChannelService.getSmsChannelPage(pageVO));
    }
    ...
    public CommonResult<List<SmsChannelSimpleRespVO>> getSimpleSmsChannelList() {
        List<SystemSmsChannel> list = smsChannelService.getSmsChannelList();
        // 排序后，返回给前端
        list.sort(Comparator.comparing(SystemSmsChannel::id));
