...
import cn.iocoder.yudao.service.service.infra.mail.MailAccountService;
...
@Tag(name = "管理后台 - 邮箱账号")
@RestController
@RequestMapping("/infra/mail-account")
public class MailAccountController {
    ...
    public CommonResult<MailAccountRespVO> getMailAccount(@RequestParam("id") Long id) {
        SystemMailAccount systemMailAccount = mailAccountService.getMailAccount(id);
        return success(MailAccountConvert.INSTANCE.convert(systemMailAccount));
    }
    ...
    public CommonResult<PageResult<MailAccountBaseVO>> getMailAccountPage(@Valid MailAccountPageReqVO pageReqVO) {
        return success(mailAccountService.getMailAccountPage(pageReqVO));
    }

    @GetMapping("/list-all-simple")
    @Operation(summary = "获得邮箱账号精简列表")
    public CommonResult<List<MailAccountSimpleRespVO>> getSimpleMailAccountList() {
        List<SystemMailAccount> list = mailAccountService.getMailAccountList();
